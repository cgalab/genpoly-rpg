...
qhull configuration warning (qh_RANDOMmax in user.h):\n\
  ...
  for (i= qh_PRINTEND;  i--;  ) {
    if (qh PRINTout[i] == qh_PRINTgeom)
      ...
      fprintf (qh ferr, "qhull input error: option 'Fp' is only used for halfspace intersection ('Hn, n, n')\n");  
      qh_errexit (qh_ERRinput, NULL, NULL);  
  ...
  if (!qh KEEPcoplanar && !qh KEEPinside && !qh ONLYgood) {
    if ((qh PRINTcoplanar && qh PRINTspheres) || printcoplanar)
      fprintf (qh ferr, "qhull input warning: options 'Fc', 'FP', and 'Gp' need option 'Qc' or 'Qi' to record coplanar/inside points\n");  
  }
