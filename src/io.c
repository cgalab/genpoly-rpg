/* io.c - Input/Output routines of qhull application
   ...
   see COPYING_Qhull.txt and io.h
...
*/
void qh_produce_output(t_triangArray *triangArray) {
  ...
  for (i= 0;  i< qh_PRINTEND;  i++)
    qh_printfacets (qh fout, qh PRINTout[i], qh facet_list, NULL, !qh_ALL, triangArray);  
  qh_allstatistics();  
  ...
  if (qh PRINTstatistics) {
    ...
    fprintf(qh ferr, "size in bytes: hashentry %ld merge %ld ridge %ld vertex %ld facet %ld\n\
...
*/
void qh_printafacet(FILE *fp, int format, facetT *facet, boolT printall, t_triangArray *triangArray) {
  ...
  case qh_PRINTtriangles:
    if (qh hull_dim == 3 && format != qh_PRINTtriangles) 
      ...
      qh_printfacetNvertex_simplicial (fp, facet, format, triangArray);  
    else
...
*/
void qh_printbegin (FILE *fp, int format, facetT *facetlist, setT *facets, boolT printall, t_triangArray *triangArray) {
  ...
  case qh_PRINTtriangles:
    ...
    if (format == qh_PRINToff)
      ...
      /* fprintf (fp, "%d\nFACETS: %d %d %d\n", num, qh printoutvar 
	       ...
	       numsimplicial + numridges, totneighbors/2);   */
    }
    FORALLpoints
      ;  
...
void qh_printfacetNvertex_simplicial(FILE *fp, facetT *facet, int format, 
  ...
  if (qh_setsize(facet->vertices) != 3)
	...
	FOREACHvertex_(facet->vertices)
	  {
	    triang.vertex[count] = (qh_pointid(vertex->point))+1;  
	    count++;  
	  }
      }
    else 
  ...
  TRaddTriang(triangArray, triang);  
...
*/
void qh_printfacets(FILE *fp, int format, facetT *facetlist, setT *facets, boolT printall, t_triangArray *triangArray) {
  int numfacets, numsimplicial, numridges, totneighbors, numcoplanars;  
  facetT *facet, **facetp;  
  setT *vertices;  
  coordT *center;  
  ...
  if (format == qh_PRINTnone)
    ...
    qh_printbegin (fp, format, facetlist, facets, printall, triangArray);  
    qh_printvoronoi (fp, format, facetlist, facets, printall);  
    qh_printend (fp, format, facetlist, facets, printall);  
  }else {
    qh_printbegin (fp, format, facetlist, facets, printall, triangArray);  
    FORALLfacet_(facetlist)
      qh_printafacet (fp, format, facet, printall, triangArray);  
    FOREACHfacet_(facets) 
      qh_printafacet (fp, format, facet, printall, triangArray);  
    qh_printend (fp, format, facetlist, facets, printall);  
  }
...
selects 'C-0' or 'Qx' and merges non-convex facets.  See \"Imprecision\n\
...
If you need triangular output:\n\
  - use option 'Ft' instead of 'Q0'.\n\
    It triangulates non-simplicial facets with added points.\n\
\n\
If you must use 'Q0', \n\
...
-printneighborhood- print neighborhood of one or two facets
  ...
  bumps qh visit_id
*/
void qh_printneighborhood (FILE *fp, int format, facetT *facetA, facetT *facetB, boolT printall, t_triangArray *triangArray) {
  ...
  qh_printfacets (fp, format, NULL, facets, printall, triangArray);  
  qh_settempfree (&facets);  
} /* printneighborhood */

/*----------------------------------------
-printpoint- prints the coordinates of a point
  prints point id if string and valid point (id != -1)
...
-readpoints- read points from qh fin into all_points
    ...
    if qh ATinfinity, adds point-at-infinity for Delaunay triangulations
returns:
