/* geom.c -- geometric routines of qhull
   ...
   see COPYING_Qhull.txt and geom.h
...
notes:
  most users Qhull library should use qh_findbestfacet() instead
  use qh_findbestnew() for exhaustive search of qh newfacet_list
...
for findfacet(), findbestfacet(), and check_bestdist()
  indicated by !newfacets and isoutside defined
  searchdist is same as check_maxout()
returns:
  best facet in neighborhood of given facet
  if noupper, skips upperDelaunay facets
...
facetT *qh_findbest (pointT *point, facetT *facet, 
  ...
  realT bestdist= -REALmax/2 /* avoid underflow */, searchdist, cutoff;  
  ...
  if (!facet->flipped) {
    *numpart= 1;            
    ...
    if (!facet->upperdelaunay || (!noupper && *dist >= qh MINoutside)) {
      bestdist= *dist;  
      bestfacet= facet;  
