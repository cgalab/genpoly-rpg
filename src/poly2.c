/* poly2.c -- implements polygons and simplices
   ...
   see COPYING_Qhull.txt, poly.h and qhull.h
...
notes:
  may not need to check near-inside points because of qh MAXcoplanar 
    and KEEPnearinside (was -DISTround)
*/
#ifndef qh_NOmerge
...
/*-------------------------------------------------
-findbestfacet- find facet or best facet below a point 
  for Delaunay triangulations, the point must be lifted to a paraboloid
    by summing the squares of the coordinates.  Do not use options 'Qbk'
    'QBk', 'QbB' or 'Qbb' since they scale the coordinates.  See
    qh_setdelaunay.
returns:
...
/*-------------------------------------------------
-findfacet_all- exhaustive search for facet below a point 
  for Delaunay triangulations, the point must be lifted to a paraboloid
    by summing the squares of the coordinates.  Do not use options 'Qbk'
    'QBk', 'QbB' or 'Qbb' since they scale the coordinates.  See
    qh_setdelaunay.
returns:
  returns first facet below point
  if point is inside, returns nearest facet
  distance to facet
  isoutside if point is outside of the hull
...
/*-------------------------------------------------
-findgood- identify good facets for qh ONLYgood and qh PRINTgood
...
/*-------------------------------------------------
-furthestnext- move facet with furthest of furthest points to qh facet_next
...
/*-------------------------------------------------
-furthestout- make furthest outside point the last point of outsideset
  clears facet->notfurthest and sets facet->furthestdist
*/
...
may remove this warning.  See also 'Limitations' in qh-impre.htm.\n", 
          minangle);  
  }
...
returns:
  vertex and distance
  if qh DELAUNAY, distance is measured in the input set
*/
...
/*-------------------------------------------------
-outcoplanar- move points from outsideset to coplanarset
...
-pointfacet- return temporary set of facets indexed by point id
  ...
  access with FOREACHfacet_i_(facets) and SETelemt_(facets, i, facetT)
...
-pointvertex- return temporary set of vertices indexed by point id
  access with FOREACHvertex_i_(vertices) and SETelemt_(vertices, i, vertexT)
