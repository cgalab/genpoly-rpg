...
{
	GHsetLine(anArray->dirtySectors, &anArray->numDirty, (hashSector *) anArray->geomHash, 
              (nodeList *) &(anArray->dirtySectors), (lineContainer *) VIgetVoid(&(anArray->lines), index), 
              index, PAgetPoint(pArray, pIndex1), 
              PAgetPoint(pArray, pIndex2), pIndex1, pIndex2, anArray->gridSize);  	
...
int EAvalidateEdge(t_edgeIArray *anArray, int lineNum, int pIndex1, int pIndex2) {
 ...
 //debug//	printf ("Validate Edge: %i (%i, %i == %i, %i)  ", lineNum, pIndex1, pIndex2, i1, i2);  
	...
	if (MIN(pIndex1, pIndex2) == MIN(i1, i2) && MAX(pIndex1, pIndex2) == MAX(i1, i2)) {    
//debug// 		printf ("[VALID]\n");  
		return 1;  
	} 
    else {
...
nodeList *EAgetPossibleIsects(t_edgeIArray *anArray, int index) {
	return GHgetPossibleIntersects (anArray->geomHash, (lineContainer *)VIgetVoid(&(anArray->lines), index));  
}
...
void EAaddEdge2(t_edgeIArray *anArray, t_pointArray *pArray, int pIndex1, int pIndex2)
{
	...
	GHsetLine(anArray->dirtySectors, &anArray->numDirty, (hashSector *)anArray->geomHash, (nodeList *)&(anArray->dirtySectors), (lineContainer *) tmp , anArray->pIndex1.curNrOfPoints, PAgetPoint(pArray, pIndex1), PAgetPoint(pArray, pIndex2), pIndex1, pIndex2, anArray->gridSize);  	
